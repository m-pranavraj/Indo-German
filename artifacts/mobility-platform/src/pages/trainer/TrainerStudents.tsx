...
export function TrainerStudents() {
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      ...
      case 'attending': return <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-200">Attending</Badge>;
      ...
      case 'dropped': return <Badge variant="outline" className="bg-slate-100 text-slate-500 border-slate-200">Dropped</Badge>;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 mt-1">Track student progress, attendance, and assessment outcomes.</p>
      </div>
            ...
            <TableBody>
              {enrollments.map((enrollment) => (
                <TableRow key={enrollment.id}>
                  <TableCell>
                    <div className="font-medium text-slate-900">{enrollment.candidateName}</div>
                    <div className="text-xs text-slate-500">ID: {enrollment.candidateId}</div>
                  </TableCell>
                  <TableCell>
                    <div className="text-sm text-slate-700">{enrollment.batchName}</div>
                  ...
                  <TableCell className="text-center">
                    <span className={`font-medium ${enrollment.attendancePercent && enrollment.attendancePercent < 75 ? 'text-red-600' : 'text-slate-700'}`}>
