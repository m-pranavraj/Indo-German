...
export function CandidateJourney() {
  ...
  const getStatusIcon = (status: string) => {
    switch(status) {
      ...
      case 'active': return <Circle className="w-6 h-6 text-amber-500 fill-amber-100" />;
      case 'pending': return <Clock className="w-6 h-6 text-slate-400" />;
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      ...
      case 'active': return <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-200">In Progress</Badge>;
      case 'pending': return <Badge variant="outline" className="bg-slate-100 text-slate-600 border-slate-200">Pending</Badge>;
      case 'locked': return <Badge variant="outline" className="bg-slate-50 text-slate-400 border-slate-200">Locked</Badge>;
      default: return null;
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div>
        <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Detailed Journey</h1>
        <p className="text-slate-500 mt-1">Track your progress through the migration corridor pathway.</p>
      </div>
        ...
        {journey.stages.map((stage, index) => {
          ...
          return (
            <div key={stage.id} className="flex relative">
              ...
              <Card className={`flex-1 mb-4 border-slate-200 shadow-sm transition-all ${isActive ? 'ring-2 ring-amber-400 border-amber-200' : ''}`}>
                <CardHeader className="py-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <CardTitle className="text-lg text-slate-800">{stage.order}. {stage.name}</CardTitle>
                      {stage.completedAt && (
